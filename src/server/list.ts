    ...
    updateStatus(newDBStatus, (updateStatusError) => {
      ...
      getOldDBStatus(user, (getSelfOldDBStatusError, oldSelfDBStatus) => {
        ...
        updateStatus(newSelfDBStatus, (updateSelfStatusError) => {
          ...
          getOldDBStatus(
            { userKey: dbStatus[otherType], userid: 'other [temp] ' + otherType },
            (getOtherOldDBStatusError, otherOldDBStatus) => {
              if (getOtherOldDBStatusError && getOtherOldDBStatusError.fatal)
                return callback(getOtherOldDBStatusError, null)
              ...
              updateStatus(newOtherDBStatus, (updateOtherStatusError) => {
                ...
                osaifuDivide(
                  newDBStatus.dbKey,
                  newSelfDBStatus.dbKey,
                  newOtherDBStatus.dbKey,
                  selfType,
                  otherType,
                  (divideError) => {
...
function osaifuDivide(duoKey, selfKey, otherKey, selfType, otherType, callback) {
  ...
  duoOsaifuDB
    ...
    .exec(async (duoFindError, duoList) => {
...
function osaifuIntegration(hostKey, clientKey, duoKey, callback) {
  ...
  hostOsaifuDB
    .find({})
    .sort({ paymentDate: -1, createdAt: -1 })
    .exec((hostFindError, hostList) => {
      if (hostFindError) return callback({ type: 'DBError', fatal: true })
      clientOsaifuDB
        .find({})
        .sort({ paymentDate: -1, createdAt: -1 })
        .exec(async (clientFindError, clientList) => {
